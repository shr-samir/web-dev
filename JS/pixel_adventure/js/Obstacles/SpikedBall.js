...
function drawSpikeBall(x, y){
     ...
     pendulumCollision(player, spike_ball)
}
...
function pendulumCollision(player, spiked_ball){
        if (onCollision(player, spiked_ball)) {
          player.spriteImg.src =
            '../../assets/main_characters/virtual_guy/hit.png'
          player.totalFrame = 7
          player.onHit()
        }
}